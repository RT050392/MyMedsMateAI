...
def generate_comprehensive_patient_analysis(patient_data):
    ...
    try:
        ...
        response = openai_client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are a comprehensive healthcare AI providing all patient insights in one efficient response."},
...
def patient_management():
    ...
    for patient_id in unique_patients:
        ...
        adherence_score = patient_full_data['adherence_score'].iloc[0]
        ...
        try:
            # Get all conditions for this patient from the full dataset
            patient_conditions = patient_full_data['Conditions'].dropna().unique()
            if len(patient_conditions) > 0:
                conditions_text = patient_conditions[0]  # Take the first unique condition
            else:
                conditions_text = "No conditions recorded"
        except (AttributeError, IndexError):
            # Fallback - try to get from the single patient row
            try:
